...
### Simple User
![This is the main menu that will be accessible from all of the views. In some instances, some additional buttons that will appear on top of these when a view provides it.](figures/MenuBarUser.png)
...
### Admin User
![Some additional buttons that will appear on top of these when a view provides it.](figures/MenuBarAdmin.png)
...
*   **Logs:** If you have audit permissions, you can browse the instance's logs here (this menu was formerly called *Audit*). See the Logs section below.
*   **API:** See description given earlier for user.
...
![List Event Actions](figures/EventActions.png)
...
*   **List Collections:** List collections — named bundles of events and other elements that can be managed and synchronised together.
*   **List Event Reports:** List the rich-text event reports on this instance.
*   **List Analyst Data:** List analyst data (notes, opinions and relationships) on this instance.
...
*   **List Event Templates:** List all of the event templates created by users with template creation rights on this instance.
*   **Add Event Template:** Create a new event template.
...
#### Galaxies
![Screenshot of top bar galaxies drop down menu](./figures/Galaxies.png)
*   **List Galaxies:** Index of [MISP Galaxies](../galaxy/) on the MISP instance.
...
*   **List Sharing Groups:** You can view the list of existing Sharing Groups to which you or your organisation have access.
...
*   **Categories & Types:** Quick overview of Attribute Categories and Types. e.g: md5 -> Payload delivery, Artifacts dropped, Payload installation, External analysis
...
*  **TAXII Servers:** Configure TAXII 2.x server endpoints to push MISP data to.
...
*   **List Auth Keys:** Shows a list of the auth keys on this instance (auth keys from the advanced auth key system) with their comments.
...
*   **Workflows:** Manage event-driven automation workflows (site admins, when enabled).
*   **Top Correlations / Correlation rules / Over-correlating values:** Tools to inspect and tune the correlation engine.
*   **Benchmarking:** Per-user and per-endpoint performance metrics (site admins, when enabled).

The **Sharing Group Blueprints**, **List Object Templates** and **Decaying Models** tools are reached from the *Global Actions* menu.

#### Logs

The *Audit* menu has been renamed **Logs** and now groups four views (see the
[administration](../administration/index.qmd#logs) chapter):

*   **Application Logs:** The legacy application log — logins/logouts, failed authentication and other application events.
*   **Audit Logs:** The detailed audit trail of data changes (shown when the audit log is enabled).
*   **Access Logs:** The per-request HTTP access log (site admins only).
*   **Search Logs:** Search the application log.

#### API

*   **OpenAPI:** The browsable OpenAPI/Swagger reference for this instance's API.
*   **REST client:** The built-in interactive REST client for building, sending and inspecting API calls.
...
![Screenshot of left menu bar](./figures/LeftMenuBar.png)
