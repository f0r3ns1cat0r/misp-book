...
You are notified by a red envelope in the upper-right corner of the screen.
![Notification](./figures/delegationwaiting.png)
Open **Global Actions → View delegation requests** to see every event that other organizations want to delegate to your organization. In this example, there is a single request from Acme Factory.
![Delegated list](./figures/delagated.png)
Opening the event shows its metadata, including a banner about the pending delegation.
![Delegated event](./figures/delegatedevent.png)
Click **View request details** to see the full request — the requesting organization, the distribution level it suggested, and its message.
![Request details](./figures/recipientdetails.png)
