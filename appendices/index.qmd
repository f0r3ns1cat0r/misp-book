...
MISP supports LDAP authentication. This manual will show how to configure LDAP authentication.

::: {.callout-warning}
The configuration keys in this appendix are being revised against the current `LdapAuth` plugin. For the authoritative, up-to-date option list and the exact settings required to enable LDAP (in particular `Security.auth => ['LdapAuth.Ldap']`), refer to the plugin README at [app/Plugin/LdapAuth/README.md](https://github.com/MISP/MISP/blob/2.5/app/Plugin/LdapAuth/README.md).
:::
