...
[More about creating and managing sharing groups](using-the-system/index.qmd#create-and-manage-sharing-groups)
...
<a id="malware-information-sharing-platform-and-threat-sharing"></a>

## MISP
**MISP** is a recursive acronym for the **MISP Intelligence Sharing Platform**. The project was originally named the *Malware Information Sharing Platform*, but as its scope grew well beyond malware to cover every kind of threat intelligence, the name was redefined recursively — in the tradition of GNU ("GNU's Not Unix"). It is commonly known simply as MISP.
...
MISP can extend an event (since version 2.4.90). This allows users to build full blown events that extend an existing event, giving way to a combined event view that includes a sum total of the event along with all extending events.
[More](extended-events/index.qmd)
...
Taxonomies that can be used in MISP (2.5) and other information sharing tools, expressed in Machine Tags (Triple Tags). A machine tag is composed of a namespace (MUST), a predicate (MUST) and an (OPTIONAL) value. Machine tags are often called triple tag due to their format.
For more details on taxonomies and classification see [the taxonomies catalogue](https://www.misp-project.org/taxonomies.html). Partial source ["Taxonomy_(general)"](https://en.wikipedia.org/wiki/Taxonomy_(general)) - [CCBYSA](https://creativecommons.org/licenses/by-sa/3.0/).
...
* Permission -> Read Only, Manage Own Events, Manage Organisation Events, Manage and Publish Organisation Events
* Site Admin, Org Admin
* Sync Actions (and Internal / Authoritative Sync Actions)
* Audit Actions, Auth key access, Regex Actions
* Tagger, Tag Editor, Template Editor, Sharing Group Editor, Delegations Access
* Sighting Creator, Object Template Editor, Galaxy Editor, Decaying Model Editor
* ZMQ publisher, Kafka publisher, Warninglist Editor, View Feed Correlations
* Analyst Data Creator, Skip OTP Reqs, Server Signing

See the [Roles](administration/index.qmd#roles) section for the full list of permissions with descriptions.
...
## Analyst Data
Collaborative annotations attached to almost any MISP element (events, attributes, objects, galaxy clusters, event reports). There are three kinds: **Notes** (free-text commentary), **Opinions** (a confidence/agreement score with an optional comment) and **Relationships** (typed links between elements). Analyst data is shared and synchronised alongside the data it annotates.

## Event Report
A rich-text (Markdown) report attached to an event, used to narrate the intelligence around the indicators. Event reports support indicator extraction, import from a URL, and PDF export.

## Decaying Model
A configurable model that scores how relevant an indicator remains over time, so stale indicators can be automatically aged out instead of being trusted indefinitely.

## Workflow
An event-driven automation graph that runs when something happens in MISP (for example when an event is published), used to block, enrich or otherwise act on data. Workflows are built from triggers and modules and can be shared as blueprints.

## Cerebrate
A companion platform for managing organisations and sharing communities; a MISP instance can synchronise organisation and sharing-group definitions from a connected Cerebrate node.

## Collection
A named bundle of MISP elements (such as events and galaxy clusters) that can be managed and synchronised as a unit.
