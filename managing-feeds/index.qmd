...
To manage feeds you first need to access the Feeds page using the **Feeds** item on the **Sync Actions** top menu:
...
The index can be filtered by **scope** using the tabs at the top: **All**, **Default** (the feeds
shipped with MISP) and **Custom** (feeds you added yourself). Selecting feeds with the checkboxes
reveals **bulk actions**, so you can enable or disable many feeds at once instead of toggling each
individually.
...
* Unpublish event: If checked, the target event is unpublished whenever the feed updates it (useful when you want to review changes before they are re-published)
...
* Headers: Any HTTP headers to pass with the feed request, as a line-break-separated list in `headername: value` format. This is how you consume a feed that requires authentication — for example an `Authorization` header carrying an API token or bearer key.
...
* Default Tag: A default tag can be added to the created event(s). You can select either a single tag or a [tag collection](../sharing/index.qmd) (a preset bundle of tags), so that every event created from the feed is consistently labelled.
