---
title: "Automation and MISP API"
---
<a id="automation-api"></a>


Almost everything you can do in the MISP web interface you can also do
programmatically through its **REST API**: creating and updating events and
attributes, searching, importing and exporting in many formats, and
administering the instance. Automation is authenticated with an API key rather
than an interactive login, so scripts and tools can talk to MISP directly. Your
personal API keys are managed under **Global Actions → My Profile → Auth Keys**.

## The MISP REST API

MISP ships a complete, self-documenting API description. Rather than relying on a
hand-maintained endpoint list, use these built-in references on your own
instance:

*   **OpenAPI / Swagger reference** — a browsable OpenAPI 3.0 specification of the
    API is served at **`/api/openapi`** (rendered with Redoc). It is the
    authoritative, versioned reference for the endpoints and their request/response
    schemas for your exact MISP version.
*   **Built-in REST client** — the interactive REST client at **`/api/rest`** lets
    you build, send and inspect API calls from within MISP itself, using your
    session for authentication. It shows the exact headers, URL and body for a
    call (handy for turning a UI action into a script) and keeps a history of your
    requests.
*   **PyMISP** — for Python, the [PyMISP](../pymisp/index.qmd) library wraps the
    API; see that chapter.

The rest of this chapter is a set of practical recipes for the most common calls.
Where an endpoint's full parameter set matters, cross-check it against
`/api/openapi` for your version, as the API evolves between releases.
...
The authentication of the automation is performed via a secure key. Make sure you keep that key secret, as it gives access to your MISP data! Manage your keys under **Global Actions > My Profile > Auth Keys**.

Pass the key in an `Authorization` header on every request. URL-based authentication (putting the key in the URL) is no longer supported.
...
![Screenshot showing the newly created auth key, which is displayed only once](./figures/create-authkey-3-authkey-displayed.png)
...
*   **403 Forbidden** — authentication failed (missing/invalid API key) or the
    key's role lacks permission for the action; also returned for a read-only key
    attempting a write.
*   **404 Not Found** — the endpoint or object does not exist (or is not visible
    to you).
*   **405 Method Not Allowed** — the HTTP method is not accepted for that
    endpoint.
*   **500 Internal Server Error** — an error occurred server-side; check the
    application logs.

Error responses carry a JSON body with `name`, `message` and the requested `url`
...
- **returnFormat**: Set the return format of the search. Can be passed as the first parameter after `restSearch` or via the JSON payload. The formats supported by `/events/restSearch` are: `attack`, `attack-sightings`, `cache`, `context`, `context-markdown`, `count`, `csv`, `hashes`, `hosts`, `json`, `kunai`, `netfilter`, `opendata`, `openioc`, `rpz`, `snort`, `stix`, `stix-json`, `stix2`, `suricata`, `text`, `xml`, `yara` and `yara-json`. The exact set can vary by version — see `/api/openapi` for the authoritative list.
...
~~~
~~~
curl --header "Authorization: YOUR API KEY" --header "Accept: application/json" --header "Content-Type: application/json" -X POST -d '{"uuid":"5a0d68b3-6da0-4ced-8233-77bb950d210f","tag":"tlp:white"}' https://<misp url>/tags/attachTagToObject
~~~
~~~
curl \
...
#### Example
~~~~
curl --header "Authorization: YOUR API KEY" --header "Accept: application/json" --header "Content-Type: application/json" -X POST -d '{"event_id":"3542","value":"1.2.3.4","category":"Network activity","type":"ip-dst"}' https://<misp url>/attributes/add/3542
~~~~
...
<a id="post-objectsdeleteobjectidharddelete"></a>
...
Returns the MISP version and the calling key's capabilities. The response also
carries the **recommended PyMISP version** for this instance
(`pymisp_recommended_version`), which supersedes the deprecated
`/servers/getPyMISPVersion` endpoint.
...
~~~~json
{
  ...
  "uuid": "5bfba94a-d0a4-4ad0-8d23-73d4011fb691",
...
~~~~
curl --header "Authorization: YOUR API KEY" --header "Accept: application/json" https://<misp url>/servers/getVersion
~~~~
...
::: {.callout-warning}
The positional-URL export endpoints described in this section (for example
`/events/xml/download/...`, `/events/csv/...`, `/events/text/...`,
`/events/rpz/...` and `restSearch` with parameters baked into the URL path) are
**legacy**. They still resolve for backward compatibility, but the canonical way
to export data is a POST to `/events/restSearch` or `/attributes/restSearch` with
a JSON body and a `returnFormat` (see [Search](#search)). Prefer restSearch for
new integrations.
:::
...
The keywords false or null should be used for optional empty parameters in the URL. Also check out the User Guide to read about the [REST API](../using-the-system/index.qmd#rest-api).
