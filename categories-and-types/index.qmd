...
<a id="attribute-categories-vs-types"></a>

### Attribute Categories vs. Types

::: {.callout-note}
This reference is derived from MISP's own `Attribute::categoryDefinitions` and
`typeDefinitions` (the authoritative `category_type_mappings`). To keep it from drifting,
regenerate it from a running instance via `GET /attributes/describeTypes.json` (or the bundled
`describeTypes.json`) rather than editing cells by hand. It reflects MISP 2.5.42
(16 categories, 194 types).
:::
...
*   **edge-extension-id**: A browser extension identifier (for example, a Microsoft Edge / Chromium extension ID)
...
*   **suricata**: An IDS rule in Suricata rule-format
...
*   **uuid**: A universally unique identifier referencing another object, event, or resource
