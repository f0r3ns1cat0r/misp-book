...
The workers tab shows the workers that MISP can use. In MISP 2.5 the background worker system is **SimpleBackgroundJobs**, managed by [Supervisor](http://supervisord.org/) and backed by Redis; the legacy CakeResque worker system has been removed. You can restart the workers from this page with the "restart all workers" button, but starting and stopping them is normally handled by Supervisor (the `misp-workers` service installed by the setup scripts).

The tab is grouped into one section per queue — MISP has 6 queues: `default`, `email`, `cache`, `prio`, `update` and `scheduler`. Each section reports **Jobs in the queue** and an overall **Queue status**, followed by a table of the worker processes currently monitoring that queue:

*   **Worker PID**: The process ID of the running worker.
*   **User**: The system user the worker runs as (typically `www-data`).
*   **Worker process**: `OK` when the process is alive.
*   **Information**: A health message (for example, "The worker appears to be healthy.").
*   **Actions**: Stop an individual worker.
...
<a id="managing-correlation-exclusions"></a>
...
<a id="browsing-logs"></a>

<a id="using-misp-logs"></a>
...
<a id="scheduling-jobs-and-recurring-jobs"></a>
...
rather than editing a fixed list, choosing a **type** and the matching action:

*   **Server** — `pull`, `push` or `cache` against a synchronisation server.
*   **Feed** — `fetch` or `cache` a feed (or all feeds).
*   **Workflow** — run an ad-hoc workflow.
*   **Periodic Summary** — send the periodic summary notification.
*   **TAXII** — push to a TAXII server.
*   **Admin** — the JSON/definition updates: `updateGalaxies`, `updateTaxonomies`,
    `updateWarningLists`, `updateNoticeLists` and `updateObjectTemplates`.
...
<a id="various-administration-tips--tricks"></a>
...
<a id="the-schdlr-worker-is-not-starting"></a>
...
<a id="increase-max-size-of-samples--other-files"></a>
...
<a id="support--feature-requests"></a>
