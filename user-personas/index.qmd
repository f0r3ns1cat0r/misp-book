...
- Automate repetitive enrichment, tagging and notification with Workflows, and reuse that automation across instances via Blueprints.
...
- Capture the team's assessment alongside shared data using Analyst Data — Notes, Opinions (confidence) and typed Relationships — and bundle an incident's events and clusters into a Collection for handover.
...
Tina, Henry, Jacob, Jay, Sarah, Priya, and Marco represent other users that are also important to us.
...
- Broker trust with client and partner instances using Cerebrate and reusable Sharing Group Blueprints, and sign the events he publishes with cryptographic keys so clients can verify their authenticity (Protected Events).
...
- Integrate with the DISARM Framework (as a galaxy, formerly AM!TT) in order to describe misinformation tactics/techniques, break an incident into techniques that can be analyzed/countered, and check for disinformation through mapping.
...
- Extend MISP for disinformation, adding object types for incidents and narratives, and using [DISARM](https://www.misp-project.org/galaxy.html) (formerly AMITT) for attack patterns.

### Priya
_**The Platform Administrator**_

Priya is a MISP site administrator at a national CERT.
She keeps the instance that her analysts and partner organisations depend on healthy, current, and trustworthy — provisioning access, wiring up sharing, and automating the tedious parts so the team can focus on the intelligence itself.
She rarely creates events herself; her product is a reliable platform.

_"My users should never have to think about the plumbing — it should just work, and it should be trustworthy by default."_

#### Role
MISP Site Administrator

#### Her primary goals are to:
- Run a secure, available, and up-to-date instance that analysts and partners can rely on.
- Make the right data reach the right people automatically, while keeping sensitive data contained.
- Reduce manual, error-prone housekeeping so the analysts' time goes into analysis.

#### She uses MISP to:
- Manage users, organisations, roles and API keys, and audit activity through the logs and user login profiles.
- Automate housekeeping and policy enforcement with Workflows (blocking checks such as "refuse to publish without a TLP tag", plus non-blocking enrichment, tagging and notifications), and share that automation as Workflow Blueprints.
- Connect the instance to the wider community: synchronisation servers, feeds, TAXII 2.x servers, and organisation/sharing-group trust brokered through Cerebrate.
- Guarantee the integrity and origin of what the instance publishes by signing events with cryptographic keys (Protected Events).
- Tune data quality centrally with Decaying Models, warninglists and noticelists.
- Keep the platform healthy by monitoring background workers and jobs, scheduled tasks, diagnostics, and the update process.

#### Her objectives are to:
- Onboard new users and partner organisations quickly and safely.
- Keep the instance patched and its reference data (galaxies, taxonomies, warninglists, noticelists) current.
- Encode sharing and quality policy once, as reusable automation, rather than relying on every analyst to remember it.
- Give partners cryptographic assurance that intelligence published by the instance is authentic and unmodified.

### Marco
_**The Operator**_

Marco is an organisation administrator at a sectoral ISAC.
He does not run the MISP server itself, but he runs his organisation's day-to-day presence on it: which feeds come in, which partners they exchange with, and what the organisation publishes to the community.
He is the person who keeps the intelligence flowing and correctly scoped every day.

_"Good sharing is boring sharing — the right data, to the right people, on time, every day."_

#### Role
Organisation Administrator / Operations

#### His primary goals are to:
- Keep a steady, current flow of relevant intelligence into and out of the organisation.
- Make sure everything the organisation shares is scoped to exactly the right audience.
- Keep his team's users productive and their data clean.

#### He uses MISP to:
- Manage the organisation's users, roles and API keys, and curate the feeds the organisation ingests and evaluates before import.
- Control distribution precisely using distribution levels, sharing groups and reusable Sharing Group Blueprints, and pseudo-anonymise sensitive publications through delegation.
- Bundle related intelligence into Collections so a whole investigation or campaign can be shared and synchronised as one unit.
- Keep detection value high by managing sightings, decaying models and warninglists, and by publishing and alerting on a controlled schedule.
- Watch the organisation's dashboards and background jobs to confirm feeds, correlations and sync are running as expected.

#### His objectives are to:
- Onboard his organisation's analysts and keep their access and API keys in order.
- Maintain trusted exchange relationships with partner organisations and communities.
- Ensure nothing over- or under-shares by encoding the organisation's distribution policy into sharing groups and blueprints.
- Provide clean, well-contextualised, and timely data to the community the organisation belongs to.
